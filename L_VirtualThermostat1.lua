...
module("L_VirtualThermostat1", package.seeall)

local _PLUGIN_NAME = "VirtualThermostat"
local _PLUGIN_VERSION = "1.0"
local _CONFIGVERSION = 010000

local MYSID = "urn:toggledbits-com:serviceId:VirtualThermostat1"
local MYTYPE = "urn:schemas-toggledbits-com:device:VirtualThermostat:1"
...
function nextRun(stepStamp, pdev)
    ...
    luup.call_delay( "virtualThermostatRunScheduledTask", delay, string.format("%d:%d", stepStamp, pdev) )
end
...
function plugin_init(dev)
    ...
    for k,v in pairs(luup.devices) do
        if v.device_type == "urn:schemas-upnp-org:device:altui:1" then
            ...
            rc,rs,jj,ra = luup.call_action("urn:upnp-org:serviceId:altui1", "RegisterPlugin", 
                { newDeviceType=MYTYPE, newScriptFile="J_VirtualThermostat1_ALTUI.js", newDeviceDrawFunc="VirtualThermostat_ALTUI.DeviceDraw" }, 
                k )
    ...
    luup.variable_watch( "virtualThermostatVarChanged", MYSID, "SetpointHeating", dev )
    luup.variable_watch( "virtualThermostatVarChanged", MYSID, "SetpointCooling", dev )
    luup.variable_watch( "virtualThermostatVarChanged", OPMODE_SID, "ModeTarget", dev )
    luup.variable_watch( "virtualThermostatVarChanged", OPMODE_SID, "ModeStatus", dev )
    luup.variable_watch( "virtualThermostatVarChanged", FANMODE_SID, "Mode", dev )
    luup.variable_watch( "virtualThermostatVarChanged", FANMODE_SID, "FanStatus", dev )
    local ts = luup.variable_get(MYSID, "TempSensors", dev) or ""
    local tst = split(ts)
    for _,ts in ipairs(tst) do
        if luup.variable_get( TEMPSENS_SID, "CurrentTemperature", tonumber(ts,10) ) ~= nil then
            luup.variable_watch( "virtualThermostatVarChanged", TEMPSENS_SID, "CurrentTemperature", tonumber(ts,10) )
        end
    end
